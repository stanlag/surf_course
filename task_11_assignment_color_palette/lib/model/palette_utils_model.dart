...
abstract class PaletteUtilsModel {
  static final List<ColorFromThePalette> listColor =
      (dataColors['colors'] as List<Map<String, String>>)
          .where((colorData) => colorData.containsKey('value'))
          .toList()
          .map((color) => ColorFromThePalette(color['name']??'', color['value']??'' ))
          .toList();

  static ColorFromThePalette getColorFromHex(ColorFromThePalette listColor) {
    ...
    try {
      ...
      listColor = ColorFromThePalette(listColor.name, TextApp.error);
